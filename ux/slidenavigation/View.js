...
Ext.define('Ext.ux.slidenavigation.View', {
    ...
    config: {
        ...
        list: {
            width: 250,
            maxDrag: null,
            itemTpl: ["<tpl if='icon'><img src='{icon}' class='x-slidenavigation-icon'/></tpl>",
                '{title}'
            ],
            grouped: true,
            items: [{
     ...
     */
    onSelect: function(list, item, eOpts, stop) {
        var me = this,
            store = list.getStore(),
        ...
        if (me.__init) {
            me.fireAction('select', [me, me._cache[index], index, item.data], func, me);
        }
    },
     ...
     */
    setContainerItem: function(nav, item, transition) {
        var container = nav.container;
        if (!transition || typeof transition != 'object') {
            container.setActiveItem(item);
        }
        else {
            container.animateActiveItem(item, transition);
        }
    },

    setListItem: function (index) {
        this.list.select(index, false, true);
    },
